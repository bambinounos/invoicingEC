...
/**
 *    \file       htdocs/custom/approval/approval_note.php
 *    \ingroup    approval
 *    \brief      File of class to send credit invoices info
 */
...
class PDF extends FPDF
{
    public function Rect_one($one_value, $profit, $logo)
    {
        ...
        $this->RoundedRect(7, 50, 101, 80, 3, '1234');
        $y = 35;
        $this->SetXY(11, $y + 3);
        ...
        $this->SetXY(11, $y + 43);
        $this->Cell(20, 45, 'Contribuyente Especial Nro:');
        $this->SetXY(11, $y + 53);
        $text = utf8_decode('OBLIGADO A LLEVAR CONTABILIDAD');
        ...
        $this->SetXY($x + 1, 15);
        $this->Cell(10, 10, utf8_decode('N O T A  D E  C R É D I T O'));
        $this->SetXY($x + 1, 25);
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(10, 10, 'No.');
        $this->SetXY($x + 1, 45);
        ...
        $this->Cell(20, 45, $text);
        $this->SetXY($x, $y + 18);
        $text = utf8_decode('Razón de Modificación:');
        $this->Cell(20, 45, $text);
        ...
        $this->SetXY($x + 168, $y + 13);
        $this->Cell(20, 45, $one_value['20']);
        $this->SetXY($x + 40, $y + 18);
        $text = utf8_decode($one_value['21']);
        $this->Cell(20, 45, $text);
        $x = 28;
        $y = 62;
    ...
    public function BasicTable($header, $header1, $data, $t_data, $payment_method, $formago, $addInfo, $addlavel, $libelle)
    {
        ...
        foreach ($data as $item) {
            ...
            $y = $y_ + $re * $i;
            $this->SetXY(7, $y);
            $this->Cell($x, $py, $item['codigoInterno'], 1, '', "C");
            $this->Cell($x, $py, $item['codigoAdicional'], 1, '', "C");
            ...
            $this->SetXY($x * 8 + 15, $y);
            $this->Cell($x, $py, abs($item['descuento']), 1, '', "C");
            $this->Cell($x, $py, abs($item['precioUnitario']), 1, '', "C");
            $this->Cell($x, $py, abs($item['precioTotalSinImpuesto']), 1, '', "C");
            $this->Ln();
            if (strlen($item['descripcion']) > 150) {
                $y_ = $y_ + $re;
                $y = $y + $re;
...
if ($db->num_rows($result) > 0) {
    ...
    if (($obj->type != 2)) { //&& ($user_info->fk_purpose != 3)
    ...
    if (is_null($obj->invoice_number)) {
        $invoice_number = str_pad($user_info->fk_note_number, 9, '0', STR_PAD_LEFT);
    } elseif (is_null($user_info->fk_note_number)) {
    ...
    while ($row = $db->fetch_array($result1)) {
        ...
        $detalle = array(
            ...
            "codigoAdicional" => $row['barcode'] ? $row['barcode'] : null, // product name
            ...
            "descuento" => number_format(abs((float) ($row['multicurrency_subprice'] * $row['qty'] - $row['multicurrency_total_ht'])), 2, '.', ''),
            ...
            "impuestos" => array("impuesto" => array(
                ...
                "baseImponible" => number_format(abs((float) $row['multicurrency_total_ht']), 2, '.', ''), // total amount for product exc. taxes
    ...
    $claveacceso = date('dmY', strtotime($obj->datef)) . '04' . $obj->profid . $user_info->fk_purpose . $warehouse_no . $sellerid . $invoice_number . '12345678' . '1';
    $claveacceso = $claveacceso . $dig->getMod11Dv(strval($claveacceso));
    if (strlen($claveacceso) != 49) {
        $mesg = $langs->trans('SEN_2') . " " . strlen($claveacceso);
        $style = 'warnings';
        exitHandler($mesg, $style, 1);
    }
    ...
    if ($temp->invoice_number > 0) {
        ...
        $mesg = "Factura(" . $temp->ref . ") no emitida";
        $style = 'warnings';
        exitHandler($mesg, $style, 1);
        exit;
    }
    ...
    $new_obj = array("infoTributaria" => array(
        ...
        "codDoc" => '04', // need to add, check table 3, It means document type, for example 04 NOTA DE CRÉDITO
        ...
        "valorModificacion" => abs(number_format((float) $obj->multicurrency_total_ttc, 2, '.', '')), // Amount (excl taxes)
        "moneda" => 'DOLAR', //$temp->multicurrency_code, // type money
        "totalConImpuestos" => $totalConImpuestos,
        "motivo" => $reason[$obj->reason_type],
    ), "detalles" => $detalles);
    $xml_data = new SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?><notaCredito id="comprobante" version="1.0.0"></notaCredito>');
    ...
    $Certificate = $xpath->evaluate('/notaCredito/ds:Signature/ds:KeyInfo')->item(0)->C14N();
    $xpath->registerNamespace('etsi', 'http://uri.etsi.org/01903/v1.3.2#');
    $SignedProperties = $xpath->evaluate('/notaCredito/ds:Signature/ds:Object/etsi:QualifyingProperties/etsi:SignedProperties')->item(0)->C14N();
    $nodes = $xpath->evaluate('/notaCredito/ds:Signature/ds:SignedInfo/ds:Reference/ds:DigestValue');
    ...
    $SignatureNode = $xpath->evaluate('/notaCredito/ds:Signature/ds:SignedInfo')->item(0)->C14N();
    openssl_sign($SignatureNode, $SignatureValue, file_get_contents('Key.pem'));
    $SignatureValue = base64_encode($SignatureValue);
    $nodes = $xpath->evaluate('/notaCredito/ds:Signature/ds:SignatureValue');
    ...
    if ($user_info->fk_purpose < 3) {
        ...
        if ($ws_approval != 'RECIBIDA') {
            if ($response->RespuestaRecepcionComprobante->estado == 'RECIBIDA') {
                ...
                if ($response_tmp->identificador == 45) {
                    $invoice_number_sql = "UPDATE " . MAIN_DB_PREFIX . "user_info SET fk_note_number=" . ((int) $invoice_number + 1) . " WHERE rowid=" . ((int) $user->id);
        ...
        if ($response_tmp->estado == 'AUTORIZADO') {
            ...
            if ($response_tmp->mensajes->mensaje->mensaje) exitHandler($res, 'errors', 1);
            exit;
        }
        $db->commit();
    }
    ...
    $one_value['10'] = 'NORMAL';
    $one_value['22'] = $user_info->fk_taxpayer == "" ? null : $user_info->fk_taxpayer;
    $t = 0.00;
    $header = array(utf8_decode('Código'), utf8_decode('Código'), 'Cantidad', utf8_decode('Descripción'), 'Descuento', 'Precio', 'Precio');
    $header1 = array('', 'Adicional', ' ', utf8_decode('Descripción'), '(USD)', 'Unitario', 'Total');
    ...
    $pdf->BasicTable($header, $header1, $detalles, $t_data, utf8_decode($payment_name[$obj->fk_mode_reglement]), $formago, $addInfo, $addlavel, $obj->libelle);
    ...
    $db->commit();
    $i = $invoice_number + 1;
    if ($invoice_number == $user_info->fk_note_number) { //&& ($user_info->fk_purpose != 3)
        $invoice_number_sql = "UPDATE " . MAIN_DB_PREFIX . "user_info SET fk_note_number=" . ((int) $i) . " WHERE rowid=" . ((int) $user->id);
        $db->query($invoice_number_sql);
    }
    $db->commit();
