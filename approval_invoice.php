...
/**
 *    \file       htdocs/custom/approval/approval_invoice.php
 *    \ingroup    approval
 *    \brief      File of class to send invoices info
 */
...
class PDF extends FPDF
{
    public function Rect_one($one_value, $profit, $logo)
    {
        ...
        $this->Image(DOL_DATA_ROOT . '/facture/temp/' . $one_value['7'] . '.png', $x - 2, 110, 90, 18);
        $x = 11;
        $y = 117;
        ...
        $text = utf8_decode('Guía Remisión:');
        $this->Cell(20, 45, $text);
    ...
    public function BasicTable($header, $header1, $data, $t_data, $payment_method, $formago, $addInfo, $addlavel, $libelle)
    {
        $this->SetFont('Arial', '', 7);
        $this->SetXY(7, 162);
        $x = 17;
        foreach ($header as $col) {
            if ($col == utf8_decode('Descripción')) {
                $this->Cell($x * 5 + 8, 9, '', 1, '', 'C');
            } else {
                $this->Cell($x, 9, '', 1, '', 'C');
            }
        }
        ...
        foreach ($data as $item) {
            ...
            $y = $y_ + $re * $i;
            $this->SetXY(7, $y);
            $this->Cell($x, $py, $item['codigoPrincipal'], 1, '', "C");
            $this->Cell($x, $py, $item['codigoAuxiliar'], 1, '', "C");
            $this->Cell($x, $py, $item['cantidad'], 1, '', "C");
            $this->Cell($x * 5 + 8, $py, '', 1, '', "C");
            $this->SetXY($x * 3 + 7, $y + 1);
            $this->MultiCell($x * 5 + 8, 3, utf8_decode($item['descripcion']), 0, 'C', '', 0);
            $this->SetXY($x * 8 + 15, $y);
            $this->Cell($x, $py, $item['precioUnitario'], 1, '', "C");
            $this->Cell($x, $py, $item['descuento'], 1, '', "C");
            $this->Cell($x, $py, $item['precioTotalSinImpuesto'], 1, '', "C");
            $this->Ln();
            if (strlen($item['descripcion']) > 150) {
                $y_ = $y_ + $re;
                $y = $y + $re;
        ...
        $this->Rect($x, $t + 7, 70, 14, '', 1);
        $this->SetXY($x, $t + 7);
        $this->Cell(60, 6, 'VALOR TOTAL SIN SUBSIDIO', 0, '', "L");
        $this->Cell(10, 6, '0.00', 0, '', "R");
        $this->SetXY($x, $t + 14);
        $this->Cell(60, 4, 'AHORRO POR SUBSIDIO:', 0, "L");
        $this->SetXY($x, $t + 17);
        $this->Cell(60, 4, utf8_decode('(Incluye IVA cuando corresponda)'), 0, "L");
        $this->SetXY($x + 50, $t + 15);
        $this->Cell(20, 6, '0.00', 0, '', "R");
        $this->SetFont('Arial', '', 10);
        $this->SetXY(7, $t + 7);
        $this->Cell(80, 5, 'Forma de Pago', 1, '', "C");
        $this->Cell(42, 5, 'Valor', 1, '', "C");
        $this->SetFont('Arial', '', 7);
        $this->SetXY(7, $t + 12);
        $this->Cell(80, 5, $payment_method, 1, '', "C");
        $this->Cell(42, 5, $formago, 1, '', "C");
        $this->SetXY(7, $t + 17);
        $this->Cell(80, 5, utf8_decode('TÉRMINOS DE PAGO'), 1, '', "C");
        $this->Cell(42, 5, $libelle, 1, '', "C");
    }
}
class modulo
{
...
if ($db->num_rows($result) > 0) {
    ...
    if (is_null($obj->invoice_number)) {
        $invoice_number = str_pad($user_info->fk_invoice_number, 9, '0', STR_PAD_LEFT);
    } elseif (is_null($user_info->fk_invoice_number)) {
    ...
    while ($row = $db->fetch_array($result1)) {
        ...
        $detalle = array(
            "codigoPrincipal" => $row['ref'], // product / service reference $row['ref']
            "codigoAuxiliar" => substr($obj->ref, -4), // product name == "" ? "No Label" : $row['label']
            "descripcion" => str_replace(array("/[\x00-\x1F\x7F-\xFF]/", "\r\n", "\n", "\r"), ' ', $description), // product / service reference
            "cantidad" => $row['qty'], // QTY
            "precioUnitario" => number_format((float) $row['subprice'], 2, '.', ''), // product price
            "descuento" => number_format((float) ($row['multicurrency_subprice'] * $row['qty'] - $row['multicurrency_total_ht']), 2, '.', ''),
            ...
            "impuestos" => array(
                "impuesto" => array(
                    ...
                    "baseImponible" => number_format((float) $row['total_ht'], 2, '.', ''), // total amount for product exc. taxes
                    "valor" => number_format((float) $row['total_tva'], 2, '.', ''), // tax value 152.67 * 12 %
                )
            ),
        );
        $j++;
        ...
        if ($row['vat_src_code']) {
            if (isset($TAX_IVA[$row['vat_src_code']]['3'])) {
                $totalConImpuesto = array(
                    "codigo" => 2, //zero($TAX_IVA[$row['vat_src_code']]['0']), // Product / Service reference
                ...
                $j++;
                $totalConImpuestos['totalImpuesto_' . $j] = $totalConImpuesto;
            }
        }
    }
    $dig = new modulo();
    ...
    $claveacceso = date('dmY', strtotime($obj->datef)) . '01' . $obj->profid . $user_info->fk_purpose . $warehouse_no . $sellerid . $invoice_number . '12345678' . '1';
    ...
    if ($profid[$obj->identification_type] == "R.U.C.") {
        $ruc = 'ruc';
    } else {
        $ruc = $profid[$obj->identification_type];
    }
    $ruc = 'ruc';
    $new_obj = array(
        "infoTributaria" => array(
            ...
            strtolower($ruc) => $obj->profid, // Company Prof ID strtolower($ruc)
            "claveAcceso" => $claveacceso, // need to add and it is unique, I said you how to make it
            "codDoc" => '01', // need to add, check table 3, It means document type, for example 01 invoice
        ...
        "infoFactura" => array(
            ...
            "tipoIdentificacionComprador" => "0" . $obj->identification_type,
            //             "guiaRemision"               => $warehouse_no . ' - ' . $sellerid . ' - ' . $invoice_number,
            "razonSocialComprador" => $obj->nom, // Third - party name
            "identificacionComprador" => $obj->siren, // Third - party Prof ID
            "direccionComprador" => $obj->address ? $obj->address : null, // Third - party address
            "totalSinImpuestos" => number_format((float) $obj->multicurrency_total_ht, 2, '.', ''), // Amount (excl taxes)
            "totalDescuento" => number_format((float) $discount, 2, '.', ''), // Discount
            "totalConImpuestos" => $totalConImpuestos,
            "propina" => $tip, // * Tip
            "importeTotal" => number_format((float) $obj->multicurrency_total_ttc, 2, '.', ''), // Amount (Inc Taxes)
            "moneda" => 'DOLAR', //$obj->multicurrency_code,                                                                                                 // * Currency, "Dolar" means USD *
            "pagos" => array(
                "pago" => array(
                    "formaPago" => $payment_method[$obj->fk_mode_reglement], // * payment code see table 24
                    "total" => number_format((float) $obj->multicurrency_total_ttc, 2, '.', ''), // invoice amount inc. taxes
                    "plazo" => $obj->nbjour, // it means credit 3 months, payment conditions
                    "unidadTiempo" => 'dias', // * time unit( months, days )
                ), // it could mix many payments type, dolibarr just has 1
            ),
        ),
        "detalles" => $detalles
    );
    $xml_data = new SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"  standalone="yes"?><factura id="comprobante" version="1.0.0"></factura>');
    ...
    $Certificate = $xpath->evaluate('/factura/ds:Signature/ds:KeyInfo')->item(0)->C14N();
    $xpath->registerNamespace('etsi', 'http://uri.etsi.org/01903/v1.3.2#');
    $SignedProperties = $xpath->evaluate('/factura/ds:Signature/ds:Object/etsi:QualifyingProperties/etsi:SignedProperties')->item(0)->C14N();
    $nodes = $xpath->evaluate('/factura/ds:Signature/ds:SignedInfo/ds:Reference/ds:DigestValue');
    ...
    $SignatureNode = $xpath->evaluate('/factura/ds:Signature/ds:SignedInfo')->item(0)->C14N();
    openssl_sign($SignatureNode, $SignatureValue, file_get_contents('Key.pem'));
    $SignatureValue = base64_encode($SignatureValue);
    $nodes = $xpath->evaluate('/factura/ds:Signature/ds:SignatureValue');
        ...
        if ($ws_approval != 'RECIBIDA') {
            if ($response->RespuestaRecepcionComprobante->estado == 'RECIBIDA') {
                ...
                if ($response_tmp->identificador == '45') {
                    $invoice_number_sql = "UPDATE " . MAIN_DB_PREFIX . "user_info SET fk_invoice_number=" . ((int) $invoice_number + 1) . " WHERE rowid=" . ((int) $user->id);
    ...
    $header1 = array('Principal', 'Auxiliar', ' ', utf8_decode('Descripción'), 'Unitario', '(USD)', 'Total');
    ...
    $one_value['18'] = $obj->address;
    $one_value['20'] = $warehouse_no . '-' . $sellerid . '-' . $invoice_number;
    $one_value['23'] = $user_info->fk_microenterprise == 'on' ? "CONTRIBUYENTE RÉGIMEN RIMPE" : null; // WS certify time
    $addlavel = array(str_pad('Dirección', 10, ' ', STR_PAD_RIGHT), str_pad('Email', 10, ' ', STR_PAD_RIGHT), str_pad('Teléfono', 10, ' ', STR_PAD_RIGHT), str_pad($obj->c_note1, 10, ' ', STR_PAD_RIGHT), str_pad($obj->c_note2, 10, ' ', STR_PAD_RIGHT));
    ...
    $temp = $langs->trans("PaymentConditionShort" . $obj->code) ? $langs->trans("PaymentConditionShort" . $obj->code) : $obj->libelle;
    $pdf->BasicTable($header, $header1, $detalles, $t_data, utf8_decode($payment_name[$obj->fk_mode_reglement]), $formago, $addInfo, $addlavel, $temp);
    ...
    $db->commit();
    $i = $invoice_number + 1;
    if ($invoice_number == $user_info->fk_invoice_number) { //&& ($user_info->fk_purpose != 3)
        $invoice_number_sql = "UPDATE " . MAIN_DB_PREFIX . "user_info SET fk_invoice_number=" . ((int) $i) . " WHERE rowid=" . ((int) $user->id);
        $db->query($invoice_number_sql);
    }
    $db->commit();
    ...
    if ($index) {
        $xml_data = new SimpleXMLElement('<ventas></ventas>');
        $sql = "SELECT * FROM " . MAIN_DB_PREFIX . "canton WHERE cid=" . $obj->canton;
        $temp = $db->query($sql);
        $temp = $db->fetch_array($temp);
        $sql = "SELECT " . MAIN_DB_PREFIX . "sri_veh_info.* FROM " . MAIN_DB_PREFIX . "sri_veh_info JOIN " . MAIN_DB_PREFIX . "facture ON " . MAIN_DB_PREFIX . "facture.vehicle = " . MAIN_DB_PREFIX . "sri_veh_info.rowid WHERE " . MAIN_DB_PREFIX . "facture.rowid =" . $index;
        $rows = $db->query($sql);
        $row = $db->fetch_object($rows);
        $txtdate = date('d/m/Y', strtotime($obj->datef));
        $current = DOL_DATA_ROOT . '/facture/' . $obj->ref . '/' . date('Ymd', strtotime($obj->datef)) . '.txt';
        $xml = $xml_data->addChild('datosRegistrador');
        $xmlAdd = $xml->addChild('numeroRUC', $obj->profid);
        $xml = $xml_data->addChild('venta');
        $xmlAdd = $xml->addChild('rucComercializador', $obj->profid);
        $xmlAdd = $xml->addChild('CAMVCpn', $row->ramv_cpn);
        $xmlAdd = $xml->addChild('serialVin', $row->serial_vin);
        $xmlAdd = $xml->addChild('nombrePropietario', $obj->nom);
        $xmlAdd = $xml->addChild('tipoIdentificacionPropietario', strlen($obj->siren) == 10 ? 'C' : 'R');
        $xmlAdd = $xml->addChild('numeroDocumentoPropietario', $obj->siren);
        $xmlAdd = $xml->addChild('tipoComprobante', '1');
        $xmlAdd = $xml->addChild('establecimientoComprobante', $warehouse_no);
        $xmlAdd = $xml->addChild('puntoEmisionComprobante', $sellerid);
        $xmlAdd = $xml->addChild('numeroComprobante', $invoice_number);
        $xmlAdd = $xml->addChild('numeroAutorizacion', $claveacceso);
        $xmlAdd = $xml->addChild('fechaVenta', $txtdate);
        $xmlAdd = $xml->addChild('precioVenta', number_format((float) $obj->multicurrency_total_ttc, 2, '.', ''));
        $xmlAdd = $xml->addChild('codigoCantonMatriculacion', $temp['cid']);
        $xmlAdd = $xml->addChild('datosDireccion');
        $xmlBdd = $xmlAdd->addChild('tipo', $obj->tipo == 0 ? 'OFICINA' : 'RESIDENCIA');
        $xmlBdd = $xmlAdd->addChild('calle', $obj->address);
        $xmlBdd = $xmlAdd->addChild('numero', $obj->numero);
        $xmlBdd = $xmlAdd->addChild('interseccion', $obj->interseccion);
        $xmlAdd = $xml->addChild('datosTelefono');
        $xmlBdd = $xmlAdd->addChild('provincia', $temp['pid']);
        $xmlBdd = $xmlAdd->addChild('numero', $obj->phone);
        $dom = dom_import_simplexml($xml_data);
        file_put_contents($current, $dom->ownerDocument->saveXML($dom->ownerDocument->documentElement));
        setEventMessage($process . '.txt' . $langs->trans(" file was created"), 'mesgs');
    }
    $db->commit();
    $db->close();
