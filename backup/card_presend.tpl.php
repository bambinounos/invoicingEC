...
if ($action == 'presend') {
	...
	$i = 0;
	print_r($object->element);
	$attachfile = array();
	if ($object->element == 'invoice_supplier') $shipping = $object->ref . '/';
	else $shipping = '';
	$handle = opendir($diroutput . '/' . get_exdir($object->id, 2, 0, 0, $object, $object->element) . $shipping);
	while (false !== ($entry = readdir($handle))) {
		if ($entry != "." && $entry != ".." && $entry != "thumbs") {
			$fileparams = $diroutput . '/' . get_exdir($object->id, 2, 0, 0, $object, $object->element) . $shipping . $entry;
			$attachfile[$i] = $fileparams; // echo "$entry\n";
			$i++;
		}
	}
	closedir($handle);
	...
	$formmail->param['fileinit'] = $attachfile;
