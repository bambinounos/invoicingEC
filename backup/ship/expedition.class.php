...
class Expedition extends CommonObject
{
	...
	public function create($user, $notrigger = 0)
	{
		...
		$sql .= ", start_date, end_date, carrier, c_name1, c_note1, c_name2, c_note2, c_name3, c_note3, c_name4, c_note4, c_name5, c_note5, c_name6, c_note6, c_name7, c_note7";
		$sql .= ", identification_type, identification_c_type";
		$sql .= ") VALUES (";
		$sql .= "'(PROV)'";
		$sql .= ", " . ((int) $conf->entity);
		...
		$sql .= ", " . (isset($this->start_date) ? "'" . $this->start_date . "'" : "null") . ",";
		$sql .= (isset($this->end_date) ? "'" . $this->end_date . "'" : "null") . ",";
		$sql .= (isset($this->carrier) ? "'" . $this->carrier . "'" : "null") . ",";
		$sql .= (isset($this->c_name1) ? "'" . $this->db->escape($this->c_name1) . "'" : "null") . ",";
		$sql .= (isset($this->c_note1) ? "'" . $this->db->escape($this->c_note1) . "'" : "null") . ",";
		$sql .= (isset($this->c_name2) ? "'" . $this->db->escape($this->c_name2) . "'" : "null") . ",";
		$sql .= (isset($this->c_note2) ? "'" . $this->db->escape($this->c_note2) . "'" : "null") . ",";
		$sql .= (isset($this->c_name3) ? "'" . $this->db->escape($this->c_name3) . "'" : "null") . ",";
		$sql .= (isset($this->c_note3) ? "'" . $this->db->escape($this->c_note3) . "'" : "null") . ",";
		$sql .= (isset($this->c_name4) ? "'" . $this->db->escape($this->c_name4) . "'" : "null") . ",";
		$sql .= (isset($this->c_note4) ? "'" . $this->db->escape($this->c_note4) . "'" : "null") . ",";
		$sql .= (isset($this->c_name5) ? "'" . $this->db->escape($this->c_name5) . "'" : "null") . ",";
		$sql .= (isset($this->c_note5) ? "'" . $this->db->escape($this->c_note5) . "'" : "null") . ",";
		$sql .= (isset($this->c_name6) ? "'" . $this->db->escape($this->c_name6) . "'" : "null") . ",";
		$sql .= (isset($this->c_note6) ? "'" . $this->db->escape($this->c_note6) . "'" : "null") . ",";
		$sql .= (isset($this->c_name7) ? "'" . $this->db->escape($this->c_name7) . "'" : "null") . ",";
		$sql .= (isset($this->c_note7) ? "'" . $this->db->escape($this->c_note7) . "'" : "null") . ",";
		$sql .= (isset($this->identification_type) ? "'" . $this->identification_type . "'" : "null") . ",";
		$sql .= (isset($this->identification_c_type) ? "'" . $this->identification_c_type . "'" : "null") . ")";
	...
	public function fetch($id, $ref = '', $ref_ext = '', $notused = '')
	{
		...
		$sql .= ", e.date_expedition as date_expedition, e.model_pdf, e.fk_address, e.date_delivery, e.carrier, e.c_note1, e.c_note2, e.c_note3, e.c_note4, e.c_note5, e.c_note6, e.c_note7, e.c_name1, e.c_name2, e.c_name3, e.c_name4, e.c_name5, e.c_name6, e.c_name7,e.invoice_number, e.identification_type, e.identification_c_type, e.tip, e.reason_type, e.warehouse, e.seller, e.start_date, e.end_date";
		$sql .= ", e.ws_time, e.claveacceso, e.ws_time_end, e.claveacceso_end";
		...
		if ($result) {
			if ($this->db->num_rows($result)) {
				...
				$this->identification_c_type	= $obj->identification_c_type;
				$this->invoice_number		= $obj->invoice_number;
				$this->tip					= $obj->tip;
				$this->carrier				= $obj->carrier;
				$this->c_note1				= $obj->c_note1;
				$this->c_note2				= $obj->c_note2;
				$this->c_note3				= $obj->c_note3;
				$this->c_note4				= $obj->c_note4;
				$this->c_note5				= $obj->c_note5;
				$this->c_note6				= $obj->c_note6;
				$this->c_note7				= $obj->c_note7;
				$this->c_name1				= $obj->c_name1;
				$this->c_name2				= $obj->c_name2;
				$this->c_name3				= $obj->c_name3;
				$this->c_name4				= $obj->c_name4;
				$this->c_name5				= $obj->c_name5;
				$this->c_name6				= $obj->c_name6;
				$this->c_name7				= $obj->c_name7;
				$this->ws_time				= $obj->ws_time;
				$this->claveacceso			= $obj->claveacceso;
				$this->ws_time_end			= $obj->ws_time_end;
				$this->claveacceso_end		= $obj->claveacceso_end;
				$this->reason_type			= $obj->reason_type;
	...
	public function update($user = null, $notrigger = 0)
	{
		...
		$sql .= " identification_type=" . (isset($this->identification_type) ? "'" . $this->identification_type . "'" : "null") . ",";
		$sql .= " invoice_number=" . (isset($this->invoice_number) ? "'" . $this->invoice_number . "'" : "null") . ",";
		$sql .= " tip=" . (isset($this->tip) ? "'" . $this->tip . "'" : "null") . ",";
		$sql .= " carrier=" . (isset($this->carrier) ? "'" . $this->carrier . "'" : "null") . ",";
		$sql .= " c_note1=" . (isset($this->c_note1) ? "'" . $this->db->escape($this->c_note1) . "'" : "null") . ",";
		$sql .= " c_note2=" . (isset($this->c_note2) ? "'" . $this->db->escape($this->c_note2) . "'" : "null") . ",";
		$sql .= " c_note3=" . (isset($this->c_note3) ? "'" . $this->db->escape($this->c_note3) . "'" : "null") . ",";
		$sql .= " c_note4=" . (isset($this->c_note4) ? "'" . $this->db->escape($this->c_note4) . "'" : "null") . ",";
		$sql .= " c_note5=" . (isset($this->c_note5) ? "'" . $this->db->escape($this->c_note5) . "'" : "null") . ",";
		$sql .= " c_note6=" . (isset($this->c_note6) ? "'" . $this->db->escape($this->c_note6) . "'" : "null") . ",";
		$sql .= " c_note7=" . (isset($this->c_note7) ? "'" . $this->db->escape($this->c_note7) . "'" : "null") . ",";
		$sql .= " c_name1=" . (isset($this->c_name1) ? "'" . $this->db->escape($this->c_name1) . "'" : "null") . ",";
		$sql .= " c_name2=" . (isset($this->c_name2) ? "'" . $this->db->escape($this->c_name2) . "'" : "null") . ",";
		$sql .= " c_name3=" . (isset($this->c_name3) ? "'" . $this->db->escape($this->c_name3) . "'" : "null") . ",";
		$sql .= " c_name4=" . (isset($this->c_name4) ? "'" . $this->db->escape($this->c_name4) . "'" : "null") . ",";
		$sql .= " c_name5=" . (isset($this->c_name5) ? "'" . $this->db->escape($this->c_name5) . "'" : "null") . ",";
		$sql .= " c_name6=" . (isset($this->c_name6) ? "'" . $this->db->escape($this->c_name6) . "'" : "null") . ",";
		$sql .= " c_name7=" . (isset($this->c_name7) ? "'" . $this->db->escape($this->c_name7) . "'" : "null") . ",";
		$sql .= " ws_time=" . (isset($this->ws_time) ? "'" . $this->ws_time . "'" : "null") . ",";
		$sql .= " claveacceso=" . (isset($this->claveacceso) ? "'" . $this->claveacceso . "'" : "null") . ",";
		$sql .= " ws_time_end=" . (isset($this->ws_time_end) ? "'" . $this->ws_time_end . "'" : "null") . ",";
		$sql .= " claveacceso_end=" . (isset($this->claveacceso_end) ? "'" . $this->claveacceso_end . "'" : "null") . ",";
		$sql .= " reason_type=" . (isset($this->reason_type) ? "'" . $this->reason_type . "'" : "null") . ",";
		$sql .= " warehouse=" . (isset($this->warehouse) ? "'" . $this->warehouse . "'" : "null") . ",";
		$sql .= " start_date=" . (dol_strlen($this->start_date) != 0 ? "'" . $this->db->idate($this->start_date) . "'" : 'null') . ",";
		$sql .= " end_date=" . (dol_strlen($this->end_date) != 0 ? "'" . $this->db->idate($this->end_date) . "'" : 'null') . ",";
		$sql .= " seller=" . (isset($this->seller) ? "'" . $this->seller . "'" : "null") . ",";
	...
	public function set_start_date($user, $start_date, $notrigger = 0)
	{
		// phpcs:enable
		if ($user->rights->expedition->creer) {
			$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition";
			$sql .= " SET start_date = " . ($start_date ? "'" . $this->db->idate($start_date) . "'" : 'null');
			$sql .= " WHERE rowid = " . ((int) $this->id);

			dol_syslog(get_class($this) . "::set_start_date", LOG_DEBUG);
			$resql = $this->db->query($sql);
			if ($resql) {
				$this->start_date = $this->db->idate($start_date);
				return 1;
			} else {
				$this->error = $this->db->error();
				return -1;
	...
	// phpcs:disable PEAR.NamingConventions.ValidFunctionName.ScopeNotCamelCaps
	/**
	 *	Set the planned Transporte date
	 *
	 *	@param      User	$user        		Objet utilisateur qui modifie
	 *	@param      int		$start_date			Date de start
	 *  @param     	int		$notrigger			1=Does not execute triggers, 0= execute triggers
	 *	@return     int         				<0 si ko, >0 si ok
	 */
	public function set_end_date($user, $end_date, $notrigger = 0)
	{
		// phpcs:enable
		if ($user->rights->expedition->creer) {
			$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition";
			$sql .= " SET end_date = " . ($end_date ? "'" . $this->db->idate($end_date) . "'" : 'null');
			$sql .= " WHERE rowid = " . ((int) $this->id);

			dol_syslog(get_class($this) . "::set_end_date", LOG_DEBUG);
			$resql = $this->db->query($sql);
			if ($resql) {
				$this->end_date = $this->db->idate($end_date);
				return 1;
			} else {
				$this->error = $this->db->error();
				return -1;
