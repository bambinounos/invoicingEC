...
$langs->loadLangs(array("sendings", "companies", "bills", 'deliveries', 'orders', 'stocks', 'other', "approval", 'propal'));
...
$objectorder_id 	= GETPOST('objectorder_id', 'int');
$id = $origin_id;
if (empty($origin_id)) {
...
$identification = array('4' => 'RUC', '5' => 'CEDULA', '6' => 'PASAPORTE', '8' => 'IDENTIFICACION DEL EXTERIOR');
$reason = array('0' => 'DEVOLUCIÓN DE MERCADERÍA', '1' => 'EMISIÓN DE FACTURA POR ERROR', '2' => 'DESCUENTO', '3' => 'CORRECCIÓN DE DATOS, VALORES O INFORMACIÓN');
...
if (empty($reshook)) {
	...
	if ($action == 'add' && $user->rights->expedition->creer) {
		...
		$object->carrier				= GETPOST('carrier', 'alpha') == '' ? "NULL" :  GETPOST('carrier', 'alpha');
		$object->c_note1				= GETPOST('c_note1', 'alpha') == '' ? "NULL" :  GETPOST('c_note1', 'alpha');
		$object->c_note2				= GETPOST('c_note2', 'alpha') == '' ? "NULL" :  GETPOST('c_note2', 'alpha');
		$object->c_note3				= GETPOST('c_note3', 'alpha') == '' ? "NULL" :  GETPOST('c_note3', 'alpha');
		$object->c_note4				= GETPOST('c_note4', 'alpha') == '' ? "NULL" :  GETPOST('c_note4', 'alpha');
		$object->c_note5				= GETPOST('c_note5', 'alpha') == '' ? "NULL" :  GETPOST('c_note5', 'alpha');
		$object->c_note6				= GETPOST('c_note6', 'alpha') == '' ? "NULL" :  GETPOST('c_note6', 'alpha');
		$object->c_note7				= GETPOST('c_note7', 'alpha') == '' ? "NULL" :  GETPOST('c_note7', 'alpha');
		$object->c_name1				= GETPOST('c_name1', 'alpha') == '' ? "NULL" :  GETPOST('c_name1', 'alpha');
		$object->c_name2				= GETPOST('c_name2', 'alpha') == '' ? "NULL" :  GETPOST('c_name2', 'alpha');
		$object->c_name3				= GETPOST('c_name3', 'alpha') == '' ? "NULL" :  GETPOST('c_name3', 'alpha');
		$object->c_name4				= GETPOST('c_name4', 'alpha') == '' ? "NULL" :  GETPOST('c_name4', 'alpha');
		$object->c_name5				= GETPOST('c_name5', 'alpha') == '' ? "NULL" :  GETPOST('c_name5', 'alpha');
		$object->c_name6				= GETPOST('c_name6', 'alpha') == '' ? "NULL" :  GETPOST('c_name6', 'alpha');
		$object->c_name7				= GETPOST('c_name7', 'alpha') == '' ? "NULL" :  GETPOST('c_name7', 'alpha');
		$object->reason_type			= GETPOST('reason_type', 'alpha') == '' ? "NULL" :  GETPOST('reason_type', 'alpha');
		$object->seller					= GETPOST('seller', 'alpha') == '' ? "NULL" :  GETPOST('seller', 'alpha');
		$object->start_date				= GETPOST('start_date', 'alpha') == '' ? "NULL" :  GETPOST('start_date', 'alpha');
		$object->end_date				= GETPOST('end_date', 'alpha') == '' ? "NULL" :  GETPOST('end_date', 'alpha');
		$object->identification_type	= GETPOST('identification_type', 'alpha') == '' ? "NULL" :  GETPOST('identification_type', 'alpha');
		$object->identification_c_type	= GETPOST('identification_c_type', 'alpha') == '' ? "NULL" :  GETPOST('identification_c_type', 'alpha');
		$object->tip					= GETPOST('tip', 'alpha') == '' ? "NULL" :  GETPOST('tip', 'alpha');
...
if ($action == 'create') {
	...
	if ($origin) {
		...
		if ($object->fetch($origin_id)) {	// This include the fetch_lines
			...
			$sql = 'SELECT * FROM ' . MAIN_DB_PREFIX . 'commande WHERE rowid=' . $object->id;
			$order = $db->query($sql);
			$order  = $db->fetch_object($order);
			print '<input type="hidden" name="c_name1" value="' . $order->c_name1 . '" />';
			print '<input type="hidden" name="c_name2" value="' . $order->c_name2 . '" />';
			print '<input type="hidden" name="c_name3" value="' . $order->c_name3 . '" />';
			print '<input type="hidden" name="c_name4" value="' . $order->c_name4 . '" />';
			print '<input type="hidden" name="c_name5" value="' . $order->c_name5 . '" />';
			print '<input type="hidden" name="c_name6" value="' . $order->c_name6 . '" />';
			print '<input type="hidden" name="c_name7" value="' . $order->c_name7 . '" />';
			print '<input type="hidden" name="c_note1" value="' . $order->c_note1 . '" />';
			print '<input type="hidden" name="c_note2" value="' . $order->c_note2 . '" />';
			print '<input type="hidden" name="c_note3" value="' . $order->c_note3 . '" />';
			print '<input type="hidden" name="c_note4" value="' . $order->c_note4 . '" />';
			print '<input type="hidden" name="c_note5" value="' . $order->c_note5 . '" />';
			print '<input type="hidden" name="c_note6" value="' . $order->c_note6 . '" />';
			print '<input type="hidden" name="c_note7" value="' . $order->c_note7 . '" />';
			print '<input type="hidden" name="c_name1" value="' . $order->c_name1 . '" />';
			print '<input type="hidden" name="start_date" value="' . $order->start_date . '">';
			print '<input type="hidden" name="end_date" value="' . $order->end_date . '">';
			print '<input type="hidden" name="carrier" value="' . $order->carrier . '">';
			print '<input type="hidden" name="identification_type" value="' . $order->identification_type . '">';
			print '<input type="hidden" name="identification_c_type" value="' . $order->identification_c_type . '">';
	...
	if ($id > 0 || !empty($ref))
		$object->fetch($id, $ref);
	$objectorder->fetch($object->$typeobject->id);
	$objectorder_id = $object->$typeobject->id;
	...
	//Insert Module Part
	if ($action == 'setidentification') {
		$identification_type = GETPOST('identification_type', 'int');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET identification_type=" . $identification_type . " WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setidentification_c') {
		$identification_c_type = GETPOST('identification_c_type', 'int');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET identification_c_type=" . $identification_c_type . " WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setreason_type') {
		$reason_type = GETPOST('reason_type', 'alpha');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET reason_type='" . $reason_type . "' WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setnote1') {
		$c_note1 = GETPOST('c_note1', 'alpha');
		$c_name1 = GETPOST('c_name1', 'alpha');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET c_note1='" . $c_note1 . "', c_name1='" . $c_name1 . "' WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setnote2') {
		$c_note2 = GETPOST('c_note2', 'alpha');
		$c_name2 = GETPOST('c_name2', 'alpha');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET c_note2='" . $c_note2 . "', c_name2='" . $c_name2 . "' WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setnote5') {
		$c_note5 = GETPOST('c_note5', 'alpha');
		$c_name5 = GETPOST('c_name5', 'alpha');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET c_note5='" . $c_note5 . "', c_name5='" . $c_name5 . "' WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setnote3') {
		$c_note3 = GETPOST('c_note3', 'alpha');
		$c_name3 = GETPOST('c_name3', 'alpha');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET c_note3='" . $c_note3 . "', c_name3='" . $c_name3 . "' WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setnote4') {
		$c_note4 = GETPOST('c_note4', 'alpha');
		$c_name4 = GETPOST('c_name4', 'alpha');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET c_note4='" . $c_note4 . "', c_name4='" . $c_name4 . "' WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setnote6') {
		$c_note6 = GETPOST('c_note6', 'alpha');
		$c_name6 = GETPOST('c_name6', 'alpha');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET c_note6='Tracking Number', c_name6='" . $c_name6 . "' WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET tracking_number='" . $c_name6 . "' WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setnote7') {
		$c_note7 = GETPOST('c_note7', 'alpha');
		$c_name7 = GETPOST('c_name7', 'alpha');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET c_note7='" . $c_note7 . "', c_name7='" . $c_name7 . "' WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		// header("Refresh:0");
	} elseif ($action == 'setstart_date') {
		// print "x ".$_POST['liv_month'].", ".$_POST['liv_day'].", ".$_POST['liv_year'];
		$start_date = dol_mktime(0, 0, 0, GETPOST('liv_month'), GETPOST('liv_day'), GETPOST('liv_year'));

		$result = $object->set_start_date($user, $start_date);
		if ($result < 0) {
			setEventMessages($object->error, $object->errors, 'errors');
		}
	} elseif ($action == 'setend_date') {
		// print "x ".$_POST['liv_month'].", ".$_POST['liv_day'].", ".$_POST['liv_year'];
		$end_date = dol_mktime(0, 0, 0, GETPOST('liv_month'), GETPOST('liv_day'), GETPOST('liv_year'));

		$result = $object->set_end_date($user, $end_date);
		if ($result < 0) {
			setEventMessages($object->error, $object->errors, 'errors');
		}
	} elseif ($action == 'setcarrier') {
		$carrier = GETPOST('carrier', 'int');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "expedition SET carrier=" . $carrier . " WHERE rowid=" . ((int) $object->id);
		$result = $db->query($sql);
		if ($result < 0) {
			setEventMessages($object->error, $object->errors, 'errors');
		}
	...
	if ($conf->approval->enabled) {
		// Start date planed
		print '<tr><td>';
		$editenable = 1;
		print $form->editfieldkey("StartdateTransport", 'start_date', '', $object, $editenable);
		print '</td><td colspan="5">';
		if ($action == 'editstart_date') {
			print '<form name="setdate" action="' . $_SERVER["PHP_SELF"] . '?id=' . $id . '" method="post">';
			print '<input type="hidden" name="token" value="' . newToken() . '">';
			print '<input type="hidden" name="action" value="setstart_date">';
			print $form->selectDate($object->start_date ? $object->start_date : -1, 'liv_', '', '', '', "setstart_date");
			print '<input type="submit" class="button" value="' . $langs->trans('Modify') . '">';
			print '</form>';
		} else {
			print $object->start_date ?  dol_print_date($object->start_date,'day') : '';
		}
		print '</td>';
		print '</tr>';
		...
		// End date planed
		print '<tr><td>';
		print $form->editfieldkey("EnddateTransport", 'end_date', '', $object, $editenable);
		print '</td><td colspan="5">';
		if ($action == 'editend_date') {
			print '<form name="setdate" action="' . $_SERVER["PHP_SELF"] . '?id=' . $id . '" method="post">';
			print '<input type="hidden" name="token" value="' . newToken() . '">';
			print '<input type="hidden" name="action" value="setend_date">';
			print $form->selectDate($object->end_date ? $object->end_date : -1, 'liv_', '', '', '', "setend_date");
			print '<input type="submit" class="button" value="' . $langs->trans('Modify') . '">';
			print '</form>';
		} else {
			print $object->end_date ?  dol_print_date($object->end_date,'day') : '';
		}
		print '</td>';
		print '</tr>';
		// Identification Type
		print '<tr style="border-bottom: solid 1px #cccccc;"><td>';
		print '<table class="nobordernopadding" width="100%"><tr><td>';
		print $langs->trans('IdentificationType');
		print '</td>';
		if ($action != 'editidentification') {
			print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editidentification&amp;id=' . $id . '">' . img_edit($langs->trans('Identification Type'), 1) . '</a></td>';
		}
		print '</tr></table>';
		print '</td><td colspan="5">';
		if ($action == 'editidentification') {
			form_identification($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->identification_type);
		} else {
			print $identification[$object->identification_type];
		}
		print '</td></tr>';
		...
		if ($object->type == '2') {
			print '<tr style="border-bottom: solid 1px #cccccc;"><td>';
			print '<table class="nobordernopadding" width="100%"><tr><td>';
			print $langs->trans('ReasonType');
			print '</td>';
			if ($action != 'editreason') {
				print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editreason&amp;id=' . $id . '">' . img_edit($langs->trans('Reason Type'), 1) . '</a></td>';
			}
			print '</tr></table>';
			print '</td><td colspan="5">';
			if ($action == 'editreason') {
				form_reson($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->reason_type);
			} else {
				print $reason[$object->reason_type];
			}
			print '</td></tr>';
		}
		...
		// Custom note1
		print '<tr style="border-bottom: solid 1px #cccccc;"  ><td>';
		print '<table class="nobordernopadding" style="width:100%"><tr><td>';
		print $langs->trans('Placa');
		print '</td>';

		if ($action != 'editnote1') {
			print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote1&amp;id=' . $id . '">' . img_edit($langs->trans('Dirección'), 1) . '</a></td>';
		}
		print '</tr></table>';
		print '</td><td>';
		if ($action == 'editnote1') {
			form_editnote($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->c_note1, $object->c_name1, "1");
		} else {
			print $object->c_name1 . '</td>';
		}
		print '</td></tr>';
		...
		print '<tr style="border-bottom: solid 1px #cccccc;"  ><td>';
		print '<table class="nobordernopadding" width="100%"><tr><td>';
		print $langs->trans('Motive');
		print '</td>';
		if (($action != 'editnote2')) {
			print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote2&amp;id=' . $id . '">' . img_edit($langs->trans('Email'), 1) . '</a></td>';
		}
		print '</tr></table>';
		print '</td><td>';
		if ($action == 'editnote2') {
			form_editnote($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->c_note2, $object->c_name2, "2");
		} else {
			print $object->c_name2 . '</td>';
		}
		print '</td></tr>';
		...
		print '<tr style="border-bottom: solid 1px #cccccc;"  ><td>';
		print '<table class="nobordernopadding" width="100%"><tr><td>';
		print $langs->trans('Documento Aduanero');
		print '</td>';
		if (($action != 'editnote3')) {
			print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote3&amp;id=' . $id . '">' . img_edit($langs->trans('Email'), 1) . '</a></td>';
		}
		print '</tr></table>';
		print '</td><td>';
		if ($action == 'editnote3') {
			form_editnote($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->c_note3, $object->c_name3, "3");
		} else {
			print $object->c_name3 . '</td>';
		}
		print '</td></tr>';
		...
		print '<tr style="border-bottom: solid 1px #cccccc;"  ><td>';
		print '<table class="nobordernopadding" width="100%"><tr><td>';
		print $langs->trans('Código Establecimiento Destino');
		print '</td>';
		if (($action != 'editnote4')) {
			print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote4&amp;id=' . $id . '">' . img_edit($langs->trans('Email'), 1) . '</a></td>';
		}
		print '</tr></table>';
		print '</td><td>';
		if ($action == 'editnote4') {
			form_editnote($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->c_note4, $object->c_name4, "4");
		} else {
			print $object->c_name4 . '</td>';
		}
		print '</td></tr>';
		...
		print '<tr style="border-bottom: solid 1px #cccccc;"  ><td>';
		print '<table class="nobordernopadding" width="100%"><tr><td>';
		print $langs->trans('Ruta');
		print '</td>';
		if (($action != 'editnote5')) {
			print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote5&amp;id=' . $id . '">' . img_edit($langs->trans('Email'), 1) . '</a></td>';
		}
		print '</tr></table>';
		print '</td><td>';
		if ($action == 'editnote5') {
			form_editnote($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->c_note5, $object->c_name5, "5");
		} else {
			print $object->c_name5 . '</td>';
		}
		print '</td></tr>';
		...
		if ($action != 'editnote6') {
			print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote6&amp;id=' . $id . '">' . img_edit($langs->trans('Trackingnumber'), 1) . '</a></td>';
		}
		print '</tr></table>';
		print '</td><td>';
		if ($action == 'editnote6') {
			form_editnote($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->c_note6, $object->c_name6, "6");
		} else {
			print $object->c_name6 . '</td>';
		}
		print '</td></tr>';
		...
		print '<tr style="border-bottom: solid 1px #cccccc;"  ><td>';
		print '<table class="nobordernopadding" width="100%"><tr><td>';
		if ($object->c_note7 == NULL)
			print $langs->trans('CustomNote');
		else
			print $object->c_note7;
		print '</td>';
		if (($action != 'editnote7')) {
			print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote7&amp;id=' . $id . '">' . img_edit($langs->trans('Email'), 1) . '</a></td>';
		}
		print '</tr></table>';
		print '</td><td>';
		if ($action == 'editnote7') {
			form_editnote($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->c_note7, $object->c_name7, "7");
		} else {
			print $object->c_name7 . '</td>';
		}
		print '</td></tr>';

		// ws_time
		if (isset($object->ws_time)) {
			print '<tr style="border-bottom: solid 1px #cccccc;"><td>';
			print '<table class="nobordernopadding" width="100%"><tr><td>';
			print $object->ws_time;
			print '</td><td class="right"><span class="badge badge-status1 badge-status" title="Test">T</span></td>';
			print '</tr></table>';
			print '</td><td colspan = 2>';
			print $object->claveacceso;
			print '</td></tr>';
		}

		// ws_time
		if (isset($object->ws_time_end)) {
			print '<tr style="border-bottom: solid 1px #cccccc;"><td>';
			print '<table class="nobordernopadding" width="100%"><tr><td>';
			print $object->ws_time_end;
			print '</td><td class="right"><span class="badge badge-status6 badge-status" title="Producción">P</span></td>';
			print '</tr></table>';
			print '</td><td colspan = 2>';
			print $object->claveacceso_end;
			print '</td></tr>';
		}
	}
	...
	print '<tr style="border-bottom: solid 1px #cccccc;"><td>';
	print '<table class="nobordernopadding" width="100%"><tr><td>';
	print $langs->trans('IdentificationCType');
	print '</td>';
	if ($action != 'editidentification_c') {
		print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editidentification_c&amp;id=' . $id . '">' . img_edit($langs->trans('Identification Type'), 1) . '</a></td>';
	}
	print '</tr></table>';
	print '</td><td colspan="5">';
	if ($action == 'editidentification_c') {
		form_identification_c($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->identification_c_type);
	} else {
		print $identification[$object->identification_c_type];
	}
	print '</td></tr>';
	// Carrier
	$sql = "SELECT rowid, nom FROM llx_societe WHERE entity=1 order by nom ASC";
	$rows = $db->query($sql);
	print '<tr style="border-bottom: solid 1px #cccccc;"><td>';
	print '<table class="nobordernopadding" width="100%"><tr><td>';
	print $langs->trans('Carrier');
	print '</td>';
	if ($action != 'editcarrier') {
		print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editcarrier&amp;id=' . $id . '">' . img_edit($langs->trans('Carrier Type'), 1) . '</a></td>';
	}
	print '</tr></table>';
	print '</td><td colspan="5">';
	if ($action == 'editcarrier') {
		form_carrier($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->carrier, $db);
	} else {
		while ($row = $db->fetch_array($rows)) {
			$object->carrier == $row['rowid'] ? print $row['nom'] : '';
		}
	}
	print '</td></tr>';
	...
	$sql = 'SELECT * FROM ' . MAIN_DB_PREFIX . 'element_element WHERE ';
	$sql .= "sourcetype = 'commande' AND targettype = 'facture' ";
	$sql .= "AND fk_source = '" . $objectorder_id . "'";
	$res = $db->query($sql);
	$re  = $db->fetch_object($res);
	$in  = $re->fk_target;
	if ($conf->approval->enabled && $object->statut != 0 && ($action != 'presend')) {
		if (!isset($conf->global->STOCK_CALCULATE_ON_SHIPMENT)) {
			$mesg = "Decrease real stocks on validation of sales order";
			$style = 'mesgs';
			$_SESSION['dol_events'][$style][] = $mesg;
		} else {
			$mesg = "Decrease real stocks on shipping validation";
			$style = 'mesgs';
			$_SESSION['dol_events'][$style][] = $mesg;
		}
		print "<div class='fichecenter'>";
		print "<table><tr><td><img src='" . DOL_URL_ROOT . "/custom/approval/img/load.gif' id='spinner' class = 'imgModal' style='display:none;'></td></tr>";
		print "<tr align='center'><td><input class='button' type='button' onclick='javascript:showHint(`$objectorder_id`, `$in`,`$id`)' value = 'Enviar'></td></tr>";
		print "</div>";
	}
...
function form_carrier($page, $selected = '', $db)
{
	global $langs;
	$sql = "SELECT rowid, nom FROM llx_societe WHERE entity=1 order by nom ASC";
	$rows = $db->query($sql);
	...
	print '<input type="hidden" name="token" value="' . newToken() . '">';
	print '<input type="hidden" id="action" name="action" value="setcarrier">';
	print '<select name="carrier" id="carrier" class="js-societe-matcher" style="width:350px"';
	while ($row = $db->fetch_array($rows)) {
		print '<option value="' . $row['rowid'] . '" ' . ($selected == $row['rowid'] ? "selected" : "") . '>' . $row['nom'] . '</option>';
	}
...
function form_identification($page, $selected = '')
{
	// phpcs:enable
	global $langs;
	print '<form method="POST" action="' . $page . '">';
	print '<input type="hidden" name="action" value="setidentification">';
	print '<input type="hidden" name="token" value="' . newToken() . '">';

	print  '<select name="identification_type" id="identification_type">
	<option value="4" ' . ($selected == "4" ? "selected" : "") . '>RUC</option>
	<option value="5" ' . ($selected == "5" ? "selected" : "") . '>CEDULA</option>
	<option value="6" ' . ($selected == "6" ? "selected" : "") . '>PASAPORTE</option>
	<option value="8" ' . ($selected == "8" ? "selected" : "") . '>IDENTIFICACION DEL EXTERIOR</option></select>';
	print '<input type="submit" class="button smallpaddingimp valignmiddle" value="' . $langs->trans("Modify") . '">';
	print '</form>';
}
function form_identification_c($page, $selected = '')
{
	// phpcs:enable
	global $langs;
	print '<form method="POST" action="' . $page . '">';
	print '<input type="hidden" name="action" value="setidentification_c">';
	print '<input type="hidden" name="token" value="' . newToken() . '">';

	print  '<select name="identification_c_type" id="identification_c_type">
	<option value="4" ' . ($selected == "4" ? "selected" : "") . '>RUC</option>
	<option value="5" ' . ($selected == "5" ? "selected" : "") . '>CEDULA</option>
	<option value="6" ' . ($selected == "6" ? "selected" : "") . '>PASAPORTE</option>
	<option value="8" ' . ($selected == "8" ? "selected" : "") . '>IDENTIFICACION DEL EXTERIOR</option></select>';
...
function form_reson($page, $selected = '')
{
	// phpcs:enable
	global $langs;
	print '<form method="POST" action="' . $page . '">';
	print '<input type="hidden" name="action" value="setreason_type">';
	print '<input type="hidden" name="token" value="' . newToken() . '">';

	print  '<select name="reason_type" id="reason_type">
	<option value="0" ' . ($selected == "0" ? "selected" : "") . '>DEVOLUCIÓN DE MERCADERÍA</option>
	<option value="1" ' . ($selected == "1" ? "selected" : "") . '>EMISIÓN DE FACTURA POR ERROR</option>
	<option value="2" ' . ($selected == "2" ? "selected" : "") . '>DESCUENTO</option>
	<option value="3" ' . ($selected == "3" ? "selected" : "") . '>CORRECCIÓN DE DATOS, VALORES O INFORMACIÓN</option></select>';
	print '<input type="submit" class="button smallpaddingimp valignmiddle" value="' . $langs->trans("Modify") . '">';
	print '</form>';
}

function form_editnote($page, $selected = '', $select = '', $id)
{
	// phpcs:enable
	global $langs;
	print '<form method="POST" action="' . $page . '">';
	print '<input type="hidden" name="action" value="setnote' . $id . '" >';
	print '<input type="hidden" name="token" value="' . newToken() . '">';
	switch ($id) {
		case '1':
			$selected = 'Placa';
		...
		case '2':
			...
			break;
		case '3':
			$selected = 'Documento Aduanero';
			break;
		case '4':
			$selected = 'Código Establecimiento Destino';
			break;
		case '5':
			$selected = 'Ruta';
			break;
		case '6':
			$selected = 'El número de rastreo';
			break;
		case '7':
			$selected ? $selected : 'Custom Note';
			break;
		default:
	}
	if ($id >= 7)
		print '<input type="text" name="c_note' . $id . '" value="' . $selected . '">';
	else
		print '<input type="hidden" name="c_note' . $id . '" value="' . $selected . '">';

	print '<input type="text" name="c_name' . $id . '" value="' . $select . '">';
...
?>
<script>
	function showHint(id, invoice_number, eid) {
		var xmlhttp = new XMLHttpRequest();
		xmlhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				window.location.href = "card.php?id=" + eid;
			}
		}
		xmlhttp.open("GET", "../custom/approval/approval_order.php?index=" + id + "&in=" + invoice_number + "&eid=" + eid, true);
