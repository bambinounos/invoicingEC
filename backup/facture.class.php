...
class Facture extends CommonInvoice
{
	...
	public function fetch($rowid, $ref = '', $ref_ext = '', $notused = '', $fetch_situation = false)
	{
		...
		$sql = 'SELECT f.rowid, f.entity, f.ref, f.ref_client, f.ref_ext, f.type, f.fk_soc, f.c_note1, f.c_note2, f.c_note3, f.c_note4, f.c_note5, f.c_name1, f.c_name2, f.c_name3, f.c_name4, f.c_name5,f.invoice_number,f.identification_type, f.tip, f.reason_type, f.warehouse, f.seller';
		$sql .= ', f.total_tva, f.localtax1, f.localtax2, f.total_ht, f.total_ttc, f.revenuestamp, f.ws_time, f.claveacceso, f.ws_time_end, f.claveacceso_end';
	 ...
	 *	@return    	array				Array of invoices ($id => array('ref'=>,'paymentornot'=>,'status'=>,'paye'=>)
	 */
	public function list_qualified_avoir_invoices_debit($socid = 0)
	{
		// phpcs:enable
		global $conf;
		...
		//	$sql.= " OR f.close_code IS NOT NULL)";	// Classee payee partiellement
		$sql .= " AND ff.type IS NULL"; // Renvoi vrai si pas facture de remplacement
		$sql .= " AND f.type <> ".self::TYPE_DEBIT_NOTE; // Exclude credit note invoices from selection

		if (!empty($conf->global->INVOICE_USE_SITUATION_DEBIT_NOTE)) {
			// Keep invoices that are not situation invoices or that are the last in serie if it is a situation invoice
			$sql .= " AND (f.type <> ".self::TYPE_SITUATION." OR f.rowid IN ";
			$sql .= '(SELECT MAX(fs.rowid)'; // This select returns several ID becasue of the group by later
			$sql .= " FROM ".MAIN_DB_PREFIX."facture as fs";
		...
		dol_syslog(get_class($this)."::list_qualified_avoir_invoices_debit", LOG_DEBUG);
		$resql = $this->db->query($sql);
		if ($resql) {
			while ($obj = $this->db->fetch_object($resql)) {
				$qualified = 0;
