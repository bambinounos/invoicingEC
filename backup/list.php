...
$langs->loadLangs(array('bills', 'companies', 'products', 'categories', 'approval'));
...
$arrayfields = array(
	...
	'f.ca' => array('label'   => "WSDL Status", 'checked' => 1, 'position' => 1000),
...
$sql .= ' f.localtax1 as total_localtax1, f.localtax2 as total_localtax2, f.claveacceso, f.claveacceso_end,';
...
if ($resql) {
	...
	// WSDL
	if ((!empty($arrayfields['f.ca']['checked'])) && ($conf->approval->enabled)) {
		print '<td class="liste_titre maxwidthonsmartphone right">';
		$liststatus = array('0'  => $langs->trans("WSDL Status"), '1'  => $langs->trans("WSDL Status"), '2'  => $langs->trans("WSDL Status"), '1,2' => $langs->trans("WSDL Status") . '+' . $langs->trans("WSDL Status"), '3'  => $langs->trans("WSDL Status"));
		print '</td>';
	}	
	// Status
	...
	if (!empty($arrayfields['f.ca']['checked'])) {
		print_liste_field_titre($arrayfields['f.ca']['label'], $_SERVER['PHP_SELF'],  "f.claveacceso_end", '', $param, 'align="center"', $sortfield, $sortorder);	
	}
	if (!empty($arrayfields['f.fk_statut']['checked'])) {
	...
	if ($num > 0) {
		...
		while ($i < $imaxinloop) {
			...
			if (!empty($arrayfields['f.ca']['checked'])) {
				...
				if ($obj->claveacceso_end) print '<span class="badge badge-status6 badge-status" title="Producción">Producción</span>';
				if ($obj->claveacceso) print '<span class="badge badge-status1 badge-status" title="Test">Test</span>';
				print "</td>";
