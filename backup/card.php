...
$langs->loadLangs(array('bills', 'companies', 'compta', 'approval', 'products', 'banks', 'main', 'withdrawals'));
...
$identification = array('4' => 'RUC', '5' => 'CEDULA', '6' => 'PASAPORTE', '7' => 'VENTA A CONSUMIDOR FINAL*', '8' => 'IDENTIFICACION DELEXTERIOR*');
...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
	...
	} elseif ($action == 'setidentification') {
		$identification_type = GETPOST('identification_type', 'int');
		$sql = "UPDATE " . MAIN_DB_PREFIX . "facture SET identification_type=" . $identification_type . " WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "facture SET reason_type='" . $reason_type . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "facture SET c_note1='" . $c_note1 . "', c_name1='" . $c_name1 . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "facture SET c_note2='" . $c_note2 . "', c_name2='" . $c_name2 . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		if (($object->type == Facture::TYPE_DEBIT_NOTE || $object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_SITUATION) && $object->paye == 0 && empty($discountcheck->id)) {
		...
		if ($canconvert) {
			...
			} elseif ($object->type == Facture::TYPE_DEBIT_NOTE) {
				$discount->description = '(DEBIT_NOTE)';
			...
			if ($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT || $object->type == Facture::TYPE_SITUATION || $object->type == Facture::TYPE_DEBIT_NOTE) {
		...
		if (GETPOST('type') == Facture::TYPE_REPLACEMENT) {
			...
			if (!$error) {
				...
				$object->ref_int 			= GETPOST('ref_int');
		...
		// Debit note invoice
		if (GETPOST('type') == Facture::TYPE_DEBIT_NOTE) {
			$sourceinvoice = GETPOST('fac_avoir', 'int');
			if (!($sourceinvoice > 0) && empty($conf->global->INVOICE_CREDIT_NOTE_STANDALONE)) {
				$error++;
				setEventMessages($langs->trans("ErrorFieldRequired", $langs->transnoentitiesnoconv("CorrectInvoice")), null, 'errors');
			...
			if (!$error) {
				...
				// Proprietes particulieres a facture avoir
				$object->fk_facture_source = $sourceinvoice > 0 ? $sourceinvoice : '';
				$object->type = Facture::TYPE_DEBIT_NOTE;
				...
				// NOTE: fields situation_percent on situation invoice are stored as cumulative values on lines (bad)
				// NOTE: fields situation_percent on credit note are stored as delta on lines (good)
				if (GETPOST('InvoiceDebitWithLines', 'int') == 1 && $id > 0) {
					if (!empty($facture_source->lines)) {
						$fk_parent_line = 0;
		...
		if (
			...
			if ($newCycle > 1) {
				...
				if (count($object->tab_next_situation_invoice) > 0) {
					foreach ($object->tab_next_situation_invoice as $next_invoice) {
						if (
							$next_invoice->type == Facture::TYPE_DEBIT_NOTE
							&& $next_invoice->situation_counter == $object->situation_counter
							&& $next_invoice->fk_facture_source == $object->id
						) {
							$linkedCreditNotesList[] = $next_invoice->id;
...
if ($action == 'create') {
	...
	if (empty($origin)) {
		if ($socid > 0) {
			...
			if (empty($conf->global->INVOICE_DISABLE_DEBIT_NOTE)) {
				// Show link for credit note
				$facids = $facturestatic->list_qualified_avoir_invoices_debit($soc->id);
				if ($facids < 0) {
					dol_print_error($db, $facturestatic->error, $facturestatic->errors);
					exit;
				}
				...
				print '<div class="tagtr listofinvoicetype"><div class="tagtd listofinvoicetype">';
				$tmp = '<input type="radio" id="radio_debitnote" name="type" value="6"' . (GETPOST('type') == 6 ? ' checked' : '');
				if ((!$optionsav && empty($conf->global->INVOICE_DEBIT_NOTE_STANDALONE)) || $invoice_predefined->id > 0) {
					$tmp .= ' disabled';
				}
				$tmp .= '> ';
				// Show credit note options only if we checked credit note
				print '<script type="text/javascript">
    			jQuery(document).ready(function() {
    				if (! jQuery("#radio_debitnote").is(":checked"))
    				{
    					jQuery("#debit_note_options").hide();
    				}
    				jQuery("#radio_debitnote").click(function() {
    					jQuery("#debit_note_options").show();
    				});
    				jQuery("#radio_standard, #radio_replacement, #radio_deposit").click(function() {
    					jQuery("#debit_note_options").hide();
    				});
				...
				$text = '<label>' . $tmp . $langs->transnoentities("InvoiceDebitAsk") . '</label> ';
				$text .= '<select class="flat valignmiddle" name="fac_avoir" id="fac_avoir"';
				if (!$optionsav || $invoice_predefined->id > 0) {
					$text .= ' disabled';
				}
				...
				$text .= '</select>';
				$desc = $form->textwithpicto($text, $langs->transnoentities("InvoiceDebitDesc"), 1, 'help', '', 0, 3);
				print $desc;

				// print '<div id="debit_note_options" class="clearboth paddingtop marginbottomonly">';
				// print '&nbsp;&nbsp;&nbsp; <input type="checkbox" name="InvoiceDebitWithLines" id="InvoiceDebitWithLines" value="1" onclick="$(\'#debit_note_options input[type=checkbox]\').not(this).prop(\'checked\', false);" ' . (GETPOST('invoiceAvoirWithLines', 'int') > 0 ? 'checked' : '') . ' /> <label for="InvoiceDebitWithLines">' . $langs->trans('InvoiceDebitWithLines') . "</label>";
				// print '<br>&nbsp;&nbsp;&nbsp; <input type="checkbox" name="InvoiceDebitWithPaymentRestAmount" id="InvoiceDebitWithPaymentRestAmount" value="1" onclick="$(\'#debit_note_options input[type=checkbox]\').not(this).prop(\'checked\', false);" ' . (GETPOST('InvoiceDebitWithPaymentRestAmount', 'int') > 0 ? 'checked' : '') . ' /> <label for="InvoiceDebitWithPaymentRestAmount">' . $langs->trans('InvoiceDebitWithPaymentRestAmount') . "</label>";
				// print '</div>';
	...
	if (isModEnabled("banque")) {
		print '<tr><td>' . $langs->trans('BankAccount') . '</td><td colspan="5">';
	...
	if ($action == 'delete') {
		...
		if ($object->type != Facture::TYPE_DEPOSIT && !empty($conf->global->STOCK_CALCULATE_ON_BILL) && $object->statut >= 1) {
			...
			if ($qualified_for_stock_change) {
				...
				$label = $object->type == Facture::TYPE_DEBIT_NOTE ? $langs->trans("SelectWarehouseForStockDecrease") : $langs->trans("SelectWarehouseForStockIncrease");
				$forcecombo = 0;
	...
	if ($action == 'valid') {
		...
		if ($object->type != Facture::TYPE_CREDIT_NOTE && $object->type != Facture::TYPE_DEBIT_NOTE && $object->total_ttc < 0) { 		// Can happen only if $conf->global->FACTURE_ENABLE_NEGATIVE is on
	...
	// Type
	print '<tr><td class="fieldname_type">' . $langs->trans('Type') . '</td><td class="fieldname_type" colspan="5">';
	...
	if (($object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEBIT_NOTE) && !empty($object->fk_facture_source)) {
	...
	if ($object->type == Facture::TYPE_DEBIT_NOTE || $object->type == Facture::TYPE_DEPOSIT) {
		$discount = new DiscountAbsolute($db);
		$result = $discount->fetch(0, $object->id);
		if ($result > 0) {
			print ' <span class="opacitymediumbycolor paddingleft">';
			$s = $langs->trans("DebitNoteConvertedIntoDiscount", '{s1}', '{s2}');
			$s = str_replace('{s1}', $object->getLibType(0), $s);
			$s = str_replace('{s2}', $discount->getNomUrl(1, 'discount'), $s);
			print $s;
			print '</span><br>';
	...
	print '</tr></table>';
	print '</td><td colspan="5">';
	if ($object->type != Facture::TYPE_CREDIT_NOTE || $object->type != Facture::TYPE_DEBIT_NOTE) {
	...
	if (isModEnabled("banque")) {
		...
		print '<table width="100%" class="nobordernopadding centpercent"><tr><td class="nowrap">';
	...
	if ($action != 'editidentification') {
		print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editidentification&amp;facid=' . $object->id . '">' . img_edit($langs->trans('Identification Type'), 1) . '</a></td>';
	}
	print '</tr></table>';
	print '</td><td colspan="5">';
	if ($action == 'editidentification') {
		form_identification($_SERVER['PHP_SELF'] . '?facid=' . $object->id, $object->identification_type);
	} else {
	...
	if ($object->type != '0') {
		...
		if ($action != 'editreason') {
			print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editreason&amp;facid=' . $object->id . '">' . img_edit($langs->trans('Reason Type'), 1) . '</a></td>';
		}
		print '</tr></table>';
		print '</td><td colspan="5">';
		if ($action == 'editreason') {
			form_reson($_SERVER['PHP_SELF'] . '?facid=' . $object->id, $object->reason_type);
		} else {
	...
	if ($conf->approval->enabled) {
		if ($object->type != 3) {
			$title1 = $langs->trans('Name');
			$title2 = $langs->trans('Value');
		} else {
			$title1 = $title2 = "&nbsp";
		}
		print '<tr class="liste_titre nodrag nodrop" style="border-bottom: solid 1px #cccccc;" ><td style="width:25%"></td>';
		print '<td style="width:30%">' . $title1 . '</td>';
		print '<td style="width:20%">' . $title2 . '</td>';
		print '<td></td>';
		print '</td></tr>';
		if ($object->type != 3) {
			// Custom note1
			...
			print $langs->trans('CustomNote') . '1';
			print '</td>';

			if ($action != 'editnote1') {
				print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote1&amp;facid=' . $object->id . '">' . img_edit($langs->trans('Dirección'), 1) . '</a></td>';
			}
			print '</tr></table>';
			print '</td><td>';
			if ($action == 'editnote1') {
				form_editnote($_SERVER['PHP_SELF'] . '?facid=' . $object->id, $object->c_note1, $object->c_name1, "1");
			} else {
				print $object->c_note1 . '</td><td>' . $object->c_name1 . '</td>';
			}
			print '</td></tr>';
			...
			print '<tr style="border-bottom: solid 1px #cccccc;"><td>';
			print '<table class="nobordernopadding" width="100%"><tr><td>';
			print $langs->trans('CustomNote') . '2';
			print '</td>';
			if (($action != 'editnote2')) {
				print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote2&amp;facid=' . $object->id . '">' . img_edit($langs->trans('Email'), 1) . '</a></td>';
			}
			print '</tr></table>';
			print '</td><td>';
			if ($action == 'editnote2') {
				form_editnote($_SERVER['PHP_SELF'] . '?facid=' . $object->id, $object->c_note2, $object->c_name2, "2");
			} else {
				print $object->c_note2 . '</td><td>' . $object->c_name2 . '</td>';
			}
			print '</td></tr>';
		}
		// ws_time
	...
	if ($object->type != Facture::TYPE_CREDIT_NOTE || $object->type != Facture::TYPE_DEBIT_NOTE) {
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				if ($invoice->type == Facture::TYPE_DEBIT_NOTE) {
					print $langs->trans("DebitNote") . ' ';
				}
				...
				print '</td></tr>';
				$i++;
				if ($invoice->type == Facture::TYPE_CREDIT_NOTE || $invoice->type == Facture::TYPE_DEBIT_NOTE) {
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			if ((($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT)
					...
					|| ($object->type == Facture::TYPE_DEBIT_NOTE && empty($discount->id))
			...
			// Validate
			if ($object->statut == Facture::STATUS_DRAFT && count($object->lines) > 0 && ((($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA || $object->type == Facture::TYPE_SITUATION) && (!empty($conf->global->FACTURE_ENABLE_NEGATIVE) || $object->total_ttc >= 0)) || ($object->type == Facture::TYPE_CREDIT_NOTE && $object->total_ttc <= 0) || ($object->type == Facture::TYPE_DEBIT_NOTE))) {
				if ($usercanvalidate) {
			...
			if ($object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEBIT_NOTE || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_SITUATION) {
			...
			if (
				$object->statut == Facture::STATUS_VALIDATED && $object->paye == 0 && $usercanissuepayment && (
					($object->type != Facture::TYPE_CREDIT_NOTE && $object->type != Facture::TYPE_DEBIT_NOTE && $object->type != Facture::TYPE_DEPOSIT && ($resteapayer <= 0 || (!empty($conf->global->INVOICE_CAN_SET_PAID_EVEN_IF_PARTIALLY_PAID) && $object->total_ttc == $resteapayer))) ||
					($object->type == Facture::TYPE_CREDIT_NOTE && $resteapayer >= 0) ||
					($object->type == Facture::TYPE_DEBIT_NOTE && $resteapayer >= 0) ||
			...
			if (($object->type == Facture::TYPE_STANDARD || ($object->type == Facture::TYPE_DEPOSIT && empty($conf->global->FACTURE_DEPOSITS_ARE_JUST_PAYMENTS)) || $object->type == Facture::TYPE_PROFORMA) && $object->statut > 0 && $usercancreate) {
				if (!$objectidnext) {
					print '<a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?socid=' . $object->socid . '&amp;fac_avoir=' . $object->id . '&amp;action=create&amp;type=6' . ($object->fk_project > 0 ? '&amp;projectid=' . $object->fk_project : '') . ($object->entity > 0 ? '&amp;originentity=' . $object->entity : '') . '">' . $langs->trans("CreateDebitNote") . '</a>';
				}
			}
	...
	$pos = strpos($object->ref, 'PROV');
	if ($conf->approval->enabled && $object->statut) {
		print "<div class='fichecenter'>";
		print "<table><tr><td><img src='../../custom/approval/img/load.gif' id='spinner' class = 'imgModal' style='display:none;'></td></tr>";
		print "<tr align='center'><td><input class='button' type='button' onclick='javascript:showHint(`$object->id`, `$object->type` )' value = 'Enviar'></td></tr>";
		print "</div>";
	}
}
...
function form_identification($page, $selected = '')
{
	...
	<option value="7" ' . ($selected == "7" ? "selected" : "") . '>VENTA A CONSUMIDOR FINAL*</option>
	<option value="8" ' . ($selected == "8" ? "selected" : "") . '>IDENTIFICACION DELEXTERIOR*</option></select>';
...
function form_reson($page, $selected = '')
{
	...
	<option value="3" ' . ($selected == "3" ? "selected" : "") . '>CORRECCIÓN DE DATOS, VALORES O INFORMACIÓN</option>
	<option value="4" ' . ($selected == "4" ? "selected" : "") . '>GASTOS POR ENVIO</option></select>';
	print '<input type="submit" class="button smallpaddingimp valignmiddle" value="' . $langs->trans("Modify") . '">';
	print '</form>';
}
...
<script>
	function showHint(id, ty) {
		...
		switch (ty) {
			case '0':
				xmlhttp.open("GET", "../../custom/approval/approval_invoice.php?index=" + id, true);
				break;
			case '2':
				xmlhttp.open("GET", "../../custom/approval/approval_note.php?index=" + id, true);
				break;
			case '6':
				xmlhttp.open("GET", "../../custom/approval/approval_debit.php?index=" + id, true);
				break;
			default:
				return;
		}
