...
if ($action == 'create') {
	...
	*/
	...
	if ($id > 0 || !empty($ref)) {
	    ...
	    // Claveacceso
		$sql = $db->query("SELECT claveacceso FROM " . MAIN_DB_PREFIX . "commande_fournisseur where rowid IN (SELECT fk_source FROM " . MAIN_DB_PREFIX . "element_element where fk_target = '" . $object->id . "' AND targettype ='invoice_supplier' ORDER BY rowid DESC)");
		$res = $db->query($sql);
