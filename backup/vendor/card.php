...
$langs->loadLangs(array('admin', 'orders', 'sendings', 'approval', 'companies', 'bills', 'propal', 'receptions', 'supplier_proposal', 'deliveries', 'products', 'stocks', 'productbatch'));
...
if ($action == 'create') {
	...
	print "</form>\n";
	$diroutput = "../../custom/approval/approval_xml.php";
	print '<br><div class="centpercent table-fiche-title ">';
	print '<form name="upload" action="' . $diroutput . '" method="post" enctype="multipart/form-data">';
	print '<input type="hidden" name="token" value="' . newToken() . '">';
	print '<table class="noborder centpercent">';
	print '<tr><td>';
	print '<input type="radio" class="radio__one" value="on" name="testing" id="radio__one" checked>&nbsp;&nbsp;CLAVE DE ACCESO&nbsp;&nbsp;';
	print '<span class="radio__one__text">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" style="width: 33%;" id="claveacceso" name="claveacceso"/>';
	print '<input type="submit" class="button valignmiddle" value="' . $langs->trans("Send") . '">';
	print '</span></td></tr><tr><td><input type="radio" value="on" class="radio__two" name="testing" id="radio__two">&nbsp;&nbsp;XML&nbsp;&nbsp;';
	print '<span class="radio__two__text">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="file" class="flat" id="addedfile" name="addedfile"/>';
	print '<input type="submit" class="button valignmiddle" value="' . $langs->trans("Send") . '"></span></td></tr></table>';
	print "</form>";
	print '</div>';
	...
	$res = $db->query($sql);
	$cla = $db->fetch_object($res);

	if (!empty($cla->claveacceso)) {
		print '<tr><td colspan = 2>';
		print 'Codigo autoriazcion : ';
		print $cla->claveacceso;
		print '</td></tr>';
	}	
...
<script>
	$(function() {
		...
		if ($('.radio__one').prop("checked")) {
			$('.radio__one__text').show();
			$('#addedfile').val('');
			$('.radio__two__text').hide();
		} else {
			$('.radio__one__text').hide();
			$('#claveacceso').val('');
			$('.radio__two__text').show();
		}
		$('.radio__one').on('click', function(e) {
			$('.radio__one__text').show();
			$('#addedfile').val('');
			$('.radio__two__text').hide();
		});
		$('.radio__two').on('click', function(e) {
			$('.radio__one__text').hide();
			$('#claveacceso').val('');
			$('.radio__two__text').show();
		});
