...
$langs->loadLangs(array('orders', 'sendings', 'approval', 'companies', 'bills', 'propal', 'deliveries', 'products', 'other'));
...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $usercancreate) {
		...
		if (!$error) {
			...
			$object->start_date = date("Y-m-d", strtotime(GETPOST('sd', 'none')));
			$object->end_date = date("Y-m-d", strtotime(GETPOST('ed', 'none')));
			$object->carrier = GETPOST('carrier', 'int');
			$object->c_name1 = GETPOST('placa', 'none');
			$object->c_note1 = 'Placa';
			$object->c_name2 = GETPOST('motive', 'none');
			$object->c_note2 = 'Motive';
			$object->c_name3 = GETPOST('aduanero', 'none');
			$object->c_note3 = 'Documento Aduanero';
			$object->c_name4 = GETPOST('destino', 'none');
			$object->c_note4 = 'Código Establecimiento Destino';
			$object->c_name5 = GETPOST('ruta', 'none');
			$object->c_note5 = 'Ruta';
			$object->c_name6 = GETPOST('c_name6', 'none');
			$object->c_note6 = 'Tracking Number';
			$object->c_name7 = GETPOST('c_name7', 'none');
			$object->c_note7 = GETPOST('c_note7', 'none');
			$object->identification_type =  GETPOST('identification_type', 'none');
			$object->identification_c_type =  GETPOST('identification_c_type', 'none');
	...
	elseif ($action == 'setidentification') {
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET identification_type=" . $identification_type . " WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET identification_c_type=" . $identification_c_type . " WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET reason_type='" . $reason_type . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET c_note1='" . $c_note1 . "', c_name1='" . $c_name1 . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET c_note2='" . $c_note2 . "', c_name2='" . $c_name2 . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET c_note5='" . $c_note5 . "', c_name5='" . $c_name5 . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET c_note3='" . $c_note3 . "', c_name3='" . $c_name3 . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET c_note4='" . $c_note4 . "', c_name4='" . $c_name4 . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET c_note6='" . $c_note6 . "', c_name6='" . $c_name6 . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
		...
		$sql = "UPDATE " . MAIN_DB_PREFIX . "commande SET c_note7='" . $c_note7 . "', c_name7='" . $c_name7 . "' WHERE rowid=" . ((int) $id);
		$result = $db->query($sql);
		if ($result < 0) {
			dol_print_error($db, $object->error);
		}
	...
	} elseif ($action == 'setstart_date' && $usercancreate) {
	...
	} elseif ($action == 'setend_date' && $usercancreate) {
		// print "x ".$_POST['liv_month'].", ".$_POST['liv_day'].", ".$_POST['liv_year'];
...
if ($action == 'create' && $usercancreate) {
	...
	if ($conf->approval->enabled) {
		// Start date planed
		print '<tr><td class="fieldrequired">' . $langs->trans('StartdateTransport') . '</td><td>';
		print $form->selectDate('', 'sd', '', '', '', "crea_commande", 1, 1);
		print '</td></tr>';
		// End date planed
		print '<tr><td class="fieldrequired">' . $langs->trans('EnddateTransport') . '</td><td>';
		print $form->selectDate('', 'ed', '', '', '', "crea_commande", 1, 1);
		print '</td></tr>';
		// Identification Type
		print '<tr><td class="fieldrequired">' . $langs->trans('IdentificationType') . '</td><td>';
		...
		if ($object->type == '2') {
			...
			if ($action != 'editreason') {
				print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editreason&amp;id=' . $object->id . '">' . img_edit($langs->trans('Reason Type'), 1) . '</a></td>';
			}
			print '</tr></table>';
		...
		// Custom Placa
		print '<tr><td class="fieldrequired">' . $langs->trans('Placa') . '</td><td>';
		print '<input type="text" name="placa" value="' . GETPOST('placa') . '"></td></tr>';
		// Custom Motive
		print '<tr><td class="fieldrequired">' . $langs->trans('Motive') . '</td><td>';
		print '<input type="text" name="motive" value="' . GETPOST('motive') . '"></td></tr>';
		// Custom Aduanero
		print '<tr><td class="fieldrequired">' . $langs->trans('Documento Aduanero') . '</td><td>';
		print '<input type="text" name="aduanero" value="' . GETPOST('aduanero') . '"></td></tr>';
		// Custom Destino
		print '<tr><td class="fieldrequired">' . $langs->trans('Código Establecimiento Destino') . '</td><td>';
		print '<input type="text" name="destino" value="' . GETPOST('destino') . '"></td></tr>';
		// Custom Ruta
		print '<tr><td class="fieldrequired">' . $langs->trans('Ruta') . '</td><td>';
		print '<input type="text" name="ruta" value="' . GETPOST('ruta') . '"></td></tr>';
		// Custom Motive
		print '<tr><td class="fieldrequired">' . $langs->trans('TrackingNumber') . '</td><td>';
		print '<input type="text" name="c_name6" value="' . GETPOST('c_name6') . '"></td></tr>';
		// Custom Motive
		print '<tr><td class="fieldrequired">' . $langs->trans('CustomNote') . ' </td><td>';
		print '<input type="text" name="c_note7" value="' . GETPOST('c_note7') . '">&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;';
		print '<input type="text" name="c_name7" value="' . GETPOST('c_name7') . '"></td></tr>';
	}
	// Carrier Identification Type   CEDULA, PASAPORTE AND IDENTIFICACION DEL EXTERIOR
	print '<tr><td class="fieldrequired">' . $langs->trans('IdentificationCType') . '</td><td>';
	...
	// Carrier
	print '<tr>';
	print '<td class="fieldrequired">' . $langs->trans('Carrier') . '</td>';
	if ($socid > 0) {
		print '<td>';
		print $soc->getNomUrl(1);
		print '<input type="hidden" name="carrier" value="' . $soc->id . '">';
		print '</td>';
	} else {
		print '<td>';
		print $form->select_company('', 'carrier', '(s.client = 1 OR s.client = 3)', 'SelectThirdParty', 0, 0, null, 0, 'minwidth300');
		// reload page to retrieve customer informations
...
function form_identification_c($page, $selected = '')
{
	...
	print  '<select name="identification_type_c" id="identification_type_c">
	<option value="4" ' . ($selected == "4" ? "selected" : "") . '>RUC</option>
	<option value="5" ' . ($selected == "5" ? "selected" : "") . '>CEDULA</option>
	<option value="6" ' . ($selected == "6" ? "selected" : "") . '>PASAPORTE</option>
	<option value="8" ' . ($selected == "8" ? "selected" : "") . '>IDENTIFICACION DEL EXTERIOR</option></select>';
...
function form_editnote($page, $selected = '', $select = '', $id)
{
	...
	switch ($id) {
		...
		case '6':
			...
			break;
		case '7':
			$selected ? $selected : 'Custom Note 1';
			break;
		default:
	}
	if ($id >= 6)
